...
const SlideInCta: React.FC<SlideInCtaProps> = ({ onGetQuote }) => {
  ...
  const rtlClasses = {
    ...
    // Positioning
    position: {
      left: isRTL ? 'left-4' : 'right-4',
      right: isRTL ? 'right-4' : 'left-4'
    },
    // Spacing (logical properties)
    spacing: {
      x: isRTL ? 'space-x-reverse space-x-3' : 'space-x-3',
      xReverse: isRTL ? 'space-x-3' : 'space-x-reverse space-x-3'
    },
    // Justify content
    justify: {
    ...
    >
      <div className="relative bg-gradient-to-br from-white via-white to-inovara-neutral/20 border border-inovara-primary/15 shadow-2xl shadow-inovara-primary/20 rounded-3xl p-6 sm:p-8 backdrop-blur-xl overflow-hidden hover:shadow-3xl hover:shadow-inovara-primary/25 transition-all duration-500">
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div className={`absolute -top-20 ${isRTL ? '-left-20' : '-right-20'} w-40 h-40 bg-gradient-to-br from-inovara-accent/10 to-inovara-secondary/10 rounded-full blur-2xl animate-pulse`}></div>
          <div className={`absolute -bottom-20 ${isRTL ? '-right-20' : '-left-20'} w-32 h-32 bg-gradient-to-tr from-inovara-primary/10 to-inovara-accent/10 rounded-full blur-2xl animate-pulse delay-1000`}></div>
        ...
        <button
          onClick={handleClose}
          className={`absolute top-4 ${rtlClasses.position.left} z-20 p-2 rounded-full bg-white/80 hover:bg-white text-inovara-primary/70 hover:text-inovara-primary transition-all duration-300 shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-inovara-accent/50`}
        ...
        <div className="relative z-10">
          {/* Header Section */}
          <div className={`${rtlClasses.textAlign.center} sm:${rtlClasses.textAlign.left} mb-8`}>
            <div className={`flex items-center justify-center sm:${rtlClasses.justify.start} ${rtlClasses.spacing.x} mb-4`}>
              <div className="w-12 h-12 bg-gradient-to-br from-inovara-primary to-inovara-secondary rounded-2xl flex items-center justify-center shadow-lg flex-shrink-0">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            <div className={`flex items-center ${rtlClasses.spacing.x} p-4 rounded-xl bg-white/60 hover:bg-white/80 transition-all duration-300 border border-inovara-primary/10`}>
              <div className="w-10 h-10 bg-gradient-to-br from-inovara-accent to-inovara-accent/80 rounded-xl flex items-center justify-center flex-shrink-0">
                <Clock className="w-5 h-5 text-white" />
              </div>
              <div className={`${rtlClasses.textAlign.left} min-w-0 flex-1`}>
          ...
          {/* CTA Button */}
          <div className={`flex justify-center sm:${rtlClasses.justify.end}`}>
            <button
              onClick={handleGetQuote}
              className={`group px-8 py-4 bg-gradient-to-r from-inovara-primary to-inovara-secondary text-white rounded-2xl font-bold text-lg shadow-2xl hover:shadow-3xl transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-inovara-accent/30 flex items-center ${rtlClasses.flexDirection.row} gap-3`}
            >
